...
if(isset($_SESSION['warehouse_login'] ) &&  $_SESSION['warehouse_login'] ){
     header('Location:Storekeeper/Warehouse_Management_Panel.php');
}
...
$query = "SELECT * FROM `user` WHERE name_login = '{$send1}' AND password = '{$send2}' AND role ='warehouse' LIMIT 1";
...
if ($send_query) {
  ...
  if ($mysqli_rows) {
    ...
    $_SESSION['name_login'] = $SQL['name_login'];
    $_SESSION['role'] = $SQL['role'];
    $_SESSION['warehouse_login'] = true;
    header('Location:Storekeeper/Warehouse_Management_Panel.php');
}else{
...
body {
 overflow: hidden;
      ...
      background: url('imag/warehouse.jpeg') no-repeat center center fixed;
...
<body >
  ...
  <div class="login-box mt-5">
    ...
    <img src="imag/warehouse.svg" alt="لوگو" class="logoIcon">
    <div class="title">ورود به سیستم انبار </div>

    <form action="warehouse_login.php" method="post">
      <div class="mb-3">
